...
todaytime2 = str(todaytime.month) + '月' + todaytime.strftime('%d') + '日'  # 日期转换str：4月17日
...
Group_failure1 = pd.DataFrame(Group_failure)
Failure_follow1 = pd.DataFrame(Failure_follow)


def make_excel(department, table1, table2):
    # 读取表格
    QuXian = pd.DataFrame({'区县': ['江州区', '扶绥县', '宁明县', '龙州县', '大新县', '凭祥市', '天等县', '合计']})
    Group_failure = pd.DataFrame(table1)
    Failure_follow = pd.DataFrame(table2)
    # 准备工作
    Failure_follow.rename(columns={'通知/移交时间': '时间', '县份': '区县'}, inplace=True)  # 改列名
...
with pd.ExcelWriter('群障报表' + '.xlsx') as writer:
    make_excel('客响', Group_failure, Failure_follow).to_excel(writer, sheet_name='汇总', startcol=0, startrow=0,
                                                             index=False, header=True)
    make_excel('传输', Group_failure, Failure_follow).to_excel(writer, sheet_name='汇总', startcol=0, startrow=11,
                                                             index=False, header=True)
    Group_failure1.loc[(Group_failure1['责任部门'] != '整体'), '责任部门'] = '整体'
    Failure_follow1.loc[(Failure_follow1['责任部门'] != '整体'), '责任部门'] = '整体'
    make_excel('整体', Group_failure1, Failure_follow1).to_excel(writer, sheet_name='汇总', startcol=0, startrow=22,
                                                               index=False, header=True)
    Data = pd.DataFrame(make_excel('整体', Group_failure1, Failure_follow1))
