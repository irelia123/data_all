...
todaytime = datetime.datetime.strptime('2021-5-11','%Y-%m-%d') #手动输入日期
todaytime1 = str(todaytime.month) + '月' + str(todaytime.day) + '日'  # 日期转换str：9月01日
#
yestday = todaytime - datetime.timedelta(days=1)  # 昨天日期
yestday1 = str(yestday.month) + '月' + str(yestday.day) + '日'  # 日期转换str：9月01日
bf_yestday = todaytime - datetime.timedelta(days=2)  # 昨天日期
bf_yestday1 = str(bf_yestday.month) + '月' + str(bf_yestday.day) + '日'  # 日期转换str：9月01日
lastmonth = todaytime - datetime.timedelta(days=30)  # 昨天日期
lastmonth = str(lastmonth.month) + '月'
#读取表格
print('正在读取数据表...')
Data_Quxian = pd.read_excel('2021年广西移动家宽装机工单完成通报0511-24点-V1.3-lsy-V7.xlsx',sheet_name='表4-区县',skiprows=1)
Data_Wangge = pd.read_excel('2021年广西移动家宽装机工单完成通报0511-24点-V1.3-lsy-V7.xlsx',sheet_name='表5-网格')
Data_all1 = pd.read_excel('2021年广西移动家宽装机工单完成通报0511-24点-V1.3-lsy-V7.xlsx',sheet_name='表1-1月日报',skiprows=1,
                         skipfooter=18)
print('正在读取广西竣工详表...')
Data_gx = pd.read_excel('家宽装机及时率报表(广西）.xlsx')
print('正在读取集团竣工详表...')
Data_jt = pd.read_excel('家宽详表（集团模板）.xlsx')
print('正在读取数据表...')
Data_all = pd.read_excel('调度中心日报0501-0510.xlsx',sheet_name=4,skiprows=1)
QX_data = pd.read_excel('程序专用对照表.xlsx',sheet_name='代维区县对照表')
WG_data = pd.read_excel('程序专用对照表.xlsx',sheet_name='网格信息')
...
Data_gx.所属网格.fillna('网格信息为空',inplace=True) #  批量替换nan
data10 =pd.merge(Data_gx,WG_data,how='inner',on=["地市","所属网格"],right_index=True)
map1 = Data_gx['所属网格'].map(lambda x: x in list(data10['所属网格'])) #两个表之间一列进行判断
map1 = pd.Dataframe(map1)
map1.rename(columns={'所属网格': '判断'}, inplace=True)  #改列名
Data_gx = pd.concat([Data_gx,map1],axis =1)
Data_gx = pd.DataFrame(Data_gx)
Data_gx.rename(columns={0: '判断'}, inplace=True)  #改列名
Data_gx.loc[(Data_gx['判断'] == False) , '所属网格'] = '网格信息为空' #网格进行判断，得到所筛选的
...
Dishi_time = Dishi_time.append(
    [{'地市': '全区', '全流程工作时长': Gx_drop[['地市', '全流程工作时长']].mean().reset_index(name='全流程工作时长').at[0, '全流程工作时长']}],
    ignore_index=True)  # 计算全区
